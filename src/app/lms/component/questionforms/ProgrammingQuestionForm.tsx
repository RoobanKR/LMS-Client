...
const handleBankSelectedQuestions = useCallback((selected: any[]) => {
  ...
  // Append the new questions to the flow, then navigate to the FIRST new one.
  // Use the live ref (not the closure's stale `flowQuestions.length`) — the
  // closure-captured value can lag behind a freshly-saved Q on Save & Continue,
  // which would otherwise put currentIndex one slot AHEAD of the first new
  // question and silently skip it.
  ...
  const newStartIndex = liveFlowLen; // position of the FIRST new question
