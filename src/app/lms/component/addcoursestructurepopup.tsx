...
// Professional Toast Component - Updated to handle auto-close
...
const showToast = {
    success: (message: string) => {
        toast(({ closeToast }) => (
            ...
            onClose: () => console.log('Toast closed automatically'), // Optional: for debugging
        });
    },
    ...
    info: (message: string) => {
        toast(({ closeToast }) => (
            ...
            position: "top-right",
            autoClose: 3000, // 3 seconds for info
            hideProgressBar: true,
            closeOnClick: false,
            pauseOnHover: true,
            draggable: true,
    ...
    warning: (message: string) => {
        toast(({ closeToast }) => (
            ...
            position: "top-right",
            autoClose: 4000, // 4 seconds for warnings
            hideProgressBar: true,
            closeOnClick: false,
            pauseOnHover: true,
            draggable: true,
        ...
        {[
            ...
            <div key={id} className={`flex items-center gap-2 px-3 py-2 rounded-lg border font-sans ${disabled ? 'bg-white border-slate-200' : 'bg-white border-slate-200 hover:border-slate-300'} transition-all duration-200 hover:shadow-sm`}>
                <input
                    type="checkbox"
                    id={`${id}-checkbox`}
                    // FIX: Use type-safe property access
                    ...
                    onChange={(e) => {
                        ...
                        // FIX: Use explicit property assignment instead of dynamic key access
                        setFormData({
                            ...formData,
                            checkboxOptions: {
        ...
        {[
            ...
            <div key={id} className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-slate-200 hover:border-slate-300 transition-all duration-200 hover:shadow-sm group font-sans">
                <input
                    ...
                    id={`content-${id}`}
                    // FIX: Use type-safe property access
                    checked={id === 'video' ? selectedContentTypes.video :
                            id === 'ppt' ? selectedContentTypes.ppt :
                            id === 'pdf' ? selectedContentTypes.pdf :
                            id === 'url' ? selectedContentTypes.url :
                            id === 'folder' ? selectedContentTypes.folder :
                            selectedContentTypes.zip}
                    onChange={(e) => {
                        const newValue = e.target.checked;
                        // FIX: Use explicit property assignment instead of dynamic key access
                        setSelectedContentTypes({ 
                            ...selectedContentTypes, 
                            ...(id === 'video' && { video: newValue }),
                            ...(id === 'ppt' && { ppt: newValue }),
                            ...(id === 'pdf' && { pdf: newValue }),
                            ...(id === 'url' && { url: newValue }),
                            ...(id === 'folder' && { folder: newValue }),
                            ...(id === 'zip' && { zip: newValue })
                        });
