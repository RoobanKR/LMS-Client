  ...
  try {
    ...
    if (isFormData) {
      ...
      if (Array.isArray(questionData)) {
        payload.questionsData = questionData;
      } else {
        payload.questionsData = [questionData];
      }
...
updateQuestion: async (
  ...
  try {
    ...
    const payload = {
      tabType,
      subcategory,
      questionData: Array.isArray(questionData) ? questionData[0] : questionData, // Send single question object
      ...(Array.isArray(questionData) ? questionData[0] : questionData) // Also include flat fields for backward compatibility
    };
  ...
  // Update question API - FIXED for better compatibility
...
// ── deleteMCQQuestion (unchanged, just for reference) ──
